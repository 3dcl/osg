...
MACRO(GENERATE_PACKAGING_TARGET package_name)
    ...
    IF(${package_name} STREQUAL "libopenscenegraph")
        SET(OSG_PACKAGE_DEPENDS "libc6, libcurl3-gnutls, libgif4, libjpeg8, libpng12-0, libtiff5, libstdc++6, libopenthreads, libx11-6, libgl1-mesa-glx | libgl1, qt5-default, zlib1g")
        IF(COLLADA_FOUND AND OSG_CPP_EXCEPTIONS_AVAILABLE)
            SET(OSG_PACKAGE_DEPENDS "${OSG_PACKAGE_DEPENDS}, libcollada14 (>= 1.4.0)")
        ENDIF()
    ELSEIF(${package_name} STREQUAL "libopenscenegraph-dev")
        SET(OSG_PACKAGE_DEPENDS "libopenscenegraph(=3.3.0), libopenthreads-dev, libgl1-mesa-dev | libgl-dev, libglu-dev")
        IF(COLLADA_FOUND AND OSG_CPP_EXCEPTIONS_AVAILABLE)
            SET(OSG_PACKAGE_DEPENDS "${OSG_PACKAGE_DEPENDS}, libcollada14-dbg (>= 1.4.0)")
        ENDIF()
    ELSEIF(${package_name} STREQUAL "libopenthreads")
        IF(UNIX)
            IF(BUILD_OPENTHREADS_WITH_QT)
                SET(OSG_PACKAGE_DEPENDS "libc6, libstdc++6, qt5-default")
            ELSE()
                SET(OSG_PACKAGE_DEPENDS "libc6, libstdc++6")
            ENDIF()
        ELSE()
            SET(OSG_PACKAGE_DEPENDS "")
        ENDIF()       
    ELSEIF(${package_name} STREQUAL "libopenthreads-dev")
        SET(OSG_PACKAGE_DEPENDS "libopenthreads")        
    ELSEIF(${package_name} STREQUAL "libopenscenegraph-ffmpeg")
        SET(OSG_PACKAGE_DEPENDS "libavcodec54 (>=6:9.1-1) | libavcodec-extra-54 (>=6:9.16), libavdevice53 (>=6:9.1-1), libavformat54 (>=6:9.1-1), libavutil52 (>=6:9.1-1), libopenscenegraph")        
    ELSEIF(${package_name} STREQUAL "libopenscenegraph-gdal")
        SET(OSG_PACKAGE_DEPENDS "libgdal1h, libopenscenegraph")      
    ELSEIF(${package_name} STREQUAL "libopenscenegraph-sdl")
        SET(OSG_PACKAGE_DEPENDS "libsdl1.2debian, libopenscenegraph")    
    ELSEIF(${package_name} STREQUAL "openscenegraph")
        SET(OSG_PACKAGE_DEPENDS "libopenscenegraph, libopenthreads")   
    ELSE()
        # fallback to all dependencies if package name is not known (should be safest)
        SET(OSG_PACKAGE_DEPENDS "libavcodec54 (>=6:9.1-1) | libavcodec-extra-54 (>=6:9.16), libavdevice53 (>=6:9.1-1), libavformat54 (>=6:9.1-1), libavutil52 (>=6:9.1-1), libc6, libcurl3-gnutls, libgdal1h, libgif4, libjpeg8, libpng12-0, libtiff5, libstdc++6, libsdl1.2debian, libx11-6, libgl1-mesa-glx | libgl1, qt5-default, zlib1g")
        IF(COLLADA_FOUND AND OSG_CPP_EXCEPTIONS_AVAILABLE)
            SET(OSG_PACKAGE_DEPENDS "${OSG_PACKAGE_DEPENDS}, libcollada14 (>= 1.4.0)")
        ENDIF()
    ENDIF()
    #TODO: hardcoded conflicts
    SET(OSG_PACKAGE_CONFLICTS 
        "openscenegraph (<< 3.3.0), libopenscenegraph99, libopenscenegraph-dev (<< 3.3.0), libopenthreads14, libopenthreads-dev (<< 3.3.0), openscenegraph-examples (<< 3.3.0), openscenegraph-plugin-citygml-static, openscenegraph-plugin-citygml-shared")
